...
通过命令行启动项目时，遇到了 **端口8080持续被占用** 的问题。这是因为：
1. 之前的Java进程可能没有完全停止
2. Windows系统端口释放需要时间
3. 可能有其他应用占用了8080端口
...
5. **运行项目**
   - 方式A：点击类名左侧的**绿色▶️图标** → 选择 `Run 'JtSpringProjectApplication'`
   - 方式B：右键点击类文件 → `Run 'JtSpringProjectApplication.main()'`
   - 方式C：按快捷键 `Shift + F10`
...
如果8080端口始终被占用，可以使用其他端口：

```powershell
# 使用8081端口
java -jar .\target\JtSpringProject-0.0.1-SNAPSHOT.jar --server.port=8081
