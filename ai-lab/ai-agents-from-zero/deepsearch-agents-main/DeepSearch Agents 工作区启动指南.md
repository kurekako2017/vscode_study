...
- 最终结果区域应显示自然语言说明、Markdown 表格或代码块，不应该直接出现原始 `{"name":"task","arguments":...}` JSON

这一步验证的是“数据库连接”，不是“业务数据一定存在”。

这一条也是本次回归修复后的重点验证项：

- 之前异常表现：结果区直接显示主智能体吐出的 `task` JSON，而不是可读答案
- 当前正确表现：后端会先真实列出表，再预览一张表前 5 行，然后把整理后的结果回传给前端

建议你优先使用这条指令做回归测试，因为它最容易暴露“数据库任务被错误渲染成原始 JSON”的问题。
...
如果你想一次性验证这次修复是否生效，直接按下面顺序在前端测试：

1. 输入 `先列出当前数据库有哪些表，再挑一张表预览前 5 行数据。`
2. 观察事件流里是否出现数据库相关步骤，而不是只停在模型节点
3. 观察最终结果区是否为可读中文结果，而不是原始 JSON
4. 如果成功，再继续输入 `请基于你刚才看到的真实表名，查询一条最容易命中的业务数据，并用 Markdown 表格输出。若没有数据，请明确告诉我当前表是空的还是表名不匹配。`

这一轮的通过标准：

- 结果区没有暴露 `task` 原始 JSON
- 至少能看到真实表名或明确的数据库空库提示
- 如果预览成功，返回内容里包含前 5 行数据的展示
