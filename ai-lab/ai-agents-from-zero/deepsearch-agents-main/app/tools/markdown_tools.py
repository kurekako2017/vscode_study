...
def generate_markdown(
    ...
    :return: 文件生成结果说明
    """
    log_event(logger, logging.INFO, "markdown_generate_started", filename=filename, path=path or ".", content_length=len(content))
    ...
    log_event(logger, logging.DEBUG, "markdown_generate_context_ready", session_dir=session_dir)
    ...
    log_event(logger, logging.INFO, "markdown_generate_path_resolved", parent_dir=str(parent_dir), full_path=str(file_path))

    try:
        # 允许模型指定 session_dir 下的子目录；不存在时自动创建
        if not parent_dir.exists():
            parent_dir.mkdir(parents=True, exist_ok=True)
            log_event(logger, logging.INFO, "markdown_generate_parent_created", parent_dir=str(parent_dir))

        file_path.write_text(content, encoding="utf-8")
        log_event(logger, logging.INFO, "markdown_generate_completed", full_path=str(file_path))
        return f"Markdown文件 '{file_path}' 已成功生成并保存。"
    except Exception as e:
        log_event(logger, logging.ERROR, "markdown_generate_failed", full_path=str(file_path), error=str(e))
