...
- `npm run dev` 的终端
- 出现 `Local: http://localhost:5173/` 的位置
...
如果这里失败，说明问题不在数据库，而在后端启动、模型配置或服务本身。
...
| `RAGFlow` 私有知识库问答 | 可选 | `RAGFLOW_API_URL`、`RAGFLOW_API_KEY` | 没有知识库服务地址或凭证 |
...
- 任务链路是否完整可用，取决于 OpenRouter、NAS MySQL，以及你是否补齐可选的 Tavily / RAGFlow / NVIDIA 配置
