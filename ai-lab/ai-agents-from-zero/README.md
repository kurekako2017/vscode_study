...
3. **配置 API Key**
   ...
   - 在 `.env` 中填入你的 API Key（如通义千问/阿里百炼、DeepSeek 等），变量名请使用标准写法（如 `QWEN_API_KEY`、`DEEPSEEK_API_KEY`）
   - 如果想启用 OpenRouter 免费兜底，可以保留 `OPENROUTER_API_KEY` 和 `OPENROUTER_MODEL=openrouter/free`
...
4. **在项目根目录运行第一个案例**
   ...
   **注意**：必须在**项目根目录**执行 `python3`，否则会读不到 `.env`。这个案例现在默认是 **DeepSeek 优先，OpenRouter free 自动兜底**。如果你只想完全离线跑，也可以改用 [Ollama 本地模型](新手入门与常见问题.md#_54-ollama-示例)（无需 Key）。
