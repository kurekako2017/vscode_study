...
- Target Hybrid Retrieval Architecture
...
- Report status=generated -> Target Approval Workflow
...
查询携带用户权限；`Keyword Retrieval (Current)` 保证当前词面命中，未来通过 `Embedding` 与 `Vector Database (Target)` 演进到 `Hybrid Retrieval (Target)`，再在 `Rerank` 后组装带引用的 Context。
