...
> **ERIP V1.0 架构口径（强制）**：正式存储 = PostgreSQL + pgvector。**禁止**将 SQLite 画为 Current。InMemory=unittest only。K8s/OTel/Qdrant/Redis/RabbitMQ=未实现/后续增强。
>
> **ERIP V1.0 架构口径**：正式主链为 登录→文書→RAG→AI分析→取締役会报告→Approval→Audit→Ledger。
> PostgreSQL 为正式 Repository；InMemory 仅 unittest。Alembic head `20260717_08_ai_runtime`。
> 下文若出现 Redis/RabbitMQ 等图示，表示**扩展方向**，不是本仓默认可运行栈。
> 数字：PG **297/6 skip** · IM **286/62 skip** · FE **116/116**。
...
本文遵守 `PROJECT_BIBLE.md`。Enterprise Retail Intelligence Platform（ERIP）V1.0 已正式交付；图中若出现 SQLite/Redis/RabbitMQ 等，须区分 **V1.0 Current（PostgreSQL+pgvector、JWT/RBAC、Audit 已交付）** 与 **后续增强/历史图**；不得把 SQLite 当 Current 主库。
...
| Audit Log | 审计日志 | 監査ログ |
| PostgreSQL + pgvector | PostgreSQL + pgvector（正式） | PostgreSQL + pgvector |
...
>
> 当前图册以 ERIP 当前实现为基础，同时包含企业级扩展架构设计。
>
...
## 审批（Approval） Workflow / 承認ワークフロー / 审批工作流
...
## 检索（Retrieval） Layer Architecture / Retrieval レイヤー構成 / 检索层架构
...
## 仓储（Repository） Backend Switch Flow / Repository バックエンド切替フロー / Repository 后端切换流程
...
## 数据（Data） Import Flow / データインポートフロー / 数据导入流程
...
说明 **PostgreSQL + pgvector 是 V1.0 正式 Repository**；历史文档中的 SQLite 表述仅作早期阶段记录，不得当作当前主存储。
...
flowchart TD
    ...
    API --> DATA["PostgreSQL + pgvector / External Data"]
    FE --> API
...
flowchart TD
    ...
    F[Data and Persistence<br/>Repository Pattern / PostgreSQL+pgvector Current正式 / InMemory test-only / pgvector 部分实现]
...
```mermaid
flowchart LR
    A[Keyword Retrieval Current] --> B[Embedding 部分实现/后续增强]
    B --> C[向量检索 部分实现/后续增强]
    C --> D[Vector Store Current<br/>pgvector]
    D --> E[Hybrid Retrieval 部分实现/后续增强]
    E --> F[LangChain 编排 辅线/非主链强制]
    F --> G[检索评估 后续增强]
...
## 32. 企业部署拓扑 / Enterprise Deployment Topology / エンタープライズデプロイトポロジー

### English

```mermaid
flowchart TD
    A[V1.0 Current 基线<br/>Docker Compose 或本地<br/>FastAPI + PostgreSQL+pgvector<br/>InMemory 仅 unittest]
    B[Internet]
...
flowchart LR
    ...
    F --> G[PostgreSQL+pgvector Current正式]
    F --> H[PostgreSQL+pgvector Current正式]
    F --> I[Vector Store Interface Planned]
    I --> J[Qdrant 等后续增强 未实现]
    D --> K[Redis Target]
    D --> L[RabbitMQ Target]
```
