...
- [6. 非功能需求](#6-非功能需求)
- [7. 系统架构](#7-系统架构)
- [7.1 Document Domain Model](#71-document-domain-model)
- [8. API设计](#8-api设计)
- [9. DB设计](#9-db设计)
...
## 7.2 Document Upload API Contract Freeze

### Current State

当前系统仍只停留在 Document Domain Model，没有 Upload API。
...
Upload API 只负责受理、校验、版本边界和事件发布；审批仍是未来独立能力。

## 7.3 Document Upload Workflow + Error Catalog + Upload Policy Freeze
...
冻结 Upload Workflow、Upload Session、Idempotency、Error Catalog、Upload Policy。

### Planned

后续 Upload API 实现必须先遵守这组冻结契约。

## 7.4 Document Upload API MVP Implementation

### Current State

`POST /api/v1/documents` 已实现同步 MVP，当前只覆盖 backend。

### Target State

同步闭环包括：multipart/form-data、metadata 校验、SHA-256 checksum、重复检测、幂等处理、Repository save、事件发布与 `201 Created` 响应。

### Planned

继续保持 `GET /api/v1/documents`、`GET /api/v1/documents/{document_id}`、`GET /api/v1/documents/{document_id}/versions`、`GET /api/v1/documents/{document_id}/chunks`、`DELETE /api/v1/documents/{document_id}` 冻结未实现；PostgreSQL Document Repository 仍只设计不实现。

## 7.5 Document Read API MVP
...
## 7.7 Document Import Pipeline MVP
...
成功导入后，文档状态推进到 `validated`；导入失败时保留错误码与错误信息。当前不创建 chunk、不做检索、不做审批。
