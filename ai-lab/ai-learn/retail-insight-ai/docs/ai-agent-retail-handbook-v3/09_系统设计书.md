...
当前项目名称保持为 `Retail Insight AI`，当前定位是零售分析领域参考实现。
...
本设计书中所有平台化能力必须按目标架构或 Planned 描述，不得写成当前已落地现状。
...
- 当前未实现完整平台化目录、文档入库、审批流、互联网检索和 PostgreSQL 主事实层。

### Target Architecture

- 未来平台目标为 `Enterprise Retail Intelligence Platform (ERIP)`。
- 所有能力围绕 Platform、Domain、Infrastructure、Frontend、Documentation、Data、Template、Test 分层演进。

### Migration Strategy

- 先冻结架构与接口。
- 再逐步替换文件输入、事务持久化、检索、审批与外部 Provider。
...
当前尚未落地完整 Retrieval and RAG Platform。

### Target State

未来 Retrieval and RAG Platform 明确包括：
...
| F-13 | 企业检索 | Keyword Retrieval（Current） / Hybrid Retrieval（Target） / Vector Database（Target） + ACL | Production Gap |
...
flowchart LR
    ...
    TS --> SSE["SSE"]
    SSE --> FE
    RA --> SEARCH["Keyword Retrieval Current / Vector Database Target"]
```
...
| Layer | Responsibility | Current | Planned | Target |
| --- | --- | --- | --- | --- |
...
| Knowledge & Retrieval Layer | 管理知识接入、检索、引用、评估与演进路径 | Keyword Retrieval（Current）、Chunk、InMemory Repository | Embedding、Vector Retrieval、Citation、Rerank、Retrieval Evaluation | Hybrid Retrieval（Target）、Vector Database（Target，pgvector first；Qdrant / Milvus extensible） |
...
- Target Retrieval 统一使用 `Hybrid Retrieval (Target)`，表示 `Keyword + Vector`。
- Target Vector Layer 统一使用 `Vector Database (Target)`，第一优先是 `pgvector`，未来可扩展 `Qdrant / Milvus`。
...
| LangGraph | Workflow Orchestration、State Machine、Planned Checkpoint | 适合表达任务流转、节点状态、分支、重试与恢复 |
...
- `LangGraph -> LangChain（Target）`：LangGraph 只调用目标态的 RAG 编排能力，不持有 RAG 组件内部细节。
- `LangChain -> Repository / Vector Store Interface（Planned）`：LangChain 未来负责 Retriever、Context Builder 和 Embedding Pipeline，但数据落盘仍由 Repository / Vector Store 边界负责。
...
- Planned：`Embedding`、`Vector Retrieval`、`Metadata Filter`、`ACL Filter`、`Context Builder` 会继续补齐。
- Target：`Hybrid Retrieval (Target)`、`Rerank`、`Citation`、`Retrieval Evaluation`、`Vector Database (Target)` 属于企业级目标能力。
...
| Capability | Current | Planned | Target |
| --- | --- | --- | --- |
