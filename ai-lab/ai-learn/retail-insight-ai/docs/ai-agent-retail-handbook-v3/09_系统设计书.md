...
> **ERIP V1.0 口径：** 正式 Repository = PostgreSQL + pgvector。`PostgreSQL + pgvector（Current / 正式）` / `PostgreSQL + pgvector（Current / 正式）` 为历史表述，不得当作当前架构。详细校正图见 `09_系统设计书_7.0_Technical_Architecture_增补_修改版.md`。
...
- [18. 部署设计](#18-部署设计)
- [19. 运维设计](#19-运维设计)
- [20. Production Gap](#20-生产差距与后续增强production-gap)
- [Appendix A Document Platform Design](#appendix-a-document-platform-design)
- [A.1 Document Domain Model](#a1-文档领域模型)
- [A.2 Upload API Contract](#a2-上传-api-契约)
- [A.3 Upload Workflow](#a3-上传工作流)
- [A.4 Import Pipeline](#a4-导入流水线)
- [A.5 Read API](#a5-读取-api)
- [A.6 Archive API](#a6-归档-api)
...
| Runtime Architecture    | FastAPI、React、JWT/RBAC、PostgreSQL+pgvector、Docker Compose（可选）、宿主 PG+Vite | |
...
flowchart LR
    ...
    RA --> RG
    RG --> DATA["PostgreSQL + pgvector（Current / 正式 Repository）；InMemory=unittest only"]
    TS --> SSE["SSE"]
    SSE --> FE
    RA --> SEARCH["Keyword Retrieval Current / Hybrid Retrieval 部分实现 / pgvector 部分实现；Qdrant=后续增强"]
```
...
| Knowledge & Retrieval Layer  | 管理知识接入、检索、引用、评估与演进路径         | Keyword Retrieval、Chunk、PostgreSQL documents/chunks；InMemory=test only                                  | Embedding、Vector Retrieval、Citation、Rerank、Retrieval Evaluation                                                  | Hybrid Retrieval、Vector Database（pgvector first；Qdrant / Milvus extensible） |
| Data & Persistence Layer     | 保存任务、事件、报告、文档、审批与未来向量事实   | Repository Pattern、PostgreSQL 正式、InMemory=unittest only                                | PostgreSQL Repository 继续扩展，向量接口先冻结                                                                       | PostgreSQL + pgvector，必要时扩展 Qdrant / Milvus                               |
...
flowchart TD
    ...
    G[Repository]
    H[PostgreSQL + pgvector Current正式]
    I[PostgreSQL+pgvector Current正式]
    J[pgvector Target]
...
flowchart LR
    ...
    T1[pgvector 部分实现；Qdrant=后续增强<br/>pgvector first]
    T2[Hybrid Retrieval 部分实现]
...
```mermaid
flowchart LR
    CUR[Current<br/>PostgreSQL+pgvector + Compose可选 + InMemory test-only]
...
- **V1.0 Current：** PostgreSQL+pgvector 正式；Docker Compose 可选验收；InMemory 仅测试；本地宿主 PG+Vite。
...
| Repository        | **PostgreSQL 正式** + InMemory test-only | （历史：SQLite first 已废止）          | PostgreSQL + pgvector + 后续扩展            |
...
- **V1.0：** 业务通过 Repository 访问 **PostgreSQL**；InMemory 仅测试。历史 SQLite/CSV 路径已不作为正式架构。
...
| P0     | PostgreSQL+pgvector（已完成 Current） | 历史 SQLite/CSV  | 持续校验、备份、恢复演练    |
