...
# 23. ERIP 的企业规模定位与阶段性架构

## 23.1 结论

ERIP 的业务背景按照 **AEON 级大型流通集团**想定，但当前实现范围不是集团全社级平台，而是：

> **大型流通集团内部的区域、事业部或经营分析部门级核心系统。**

因此，当前 ERIP 既不是小企业内部工具，也不能表述为已经支持整个大型集团全员、全门店同时使用。
...
- Evidence Gate。
- 幂等控制。
- Provider 与 Repository 抽象。

因此，从应用能力和治理要求看，ERIP 已达到：
...
这一阶段可以服务数十、数百乃至逐步扩展到数千名内部用户，但真实容量必须通过性能测试确认，不能仅根据架构图承诺。
...
这类有限服务数量的容器化企业应用，与 ECS Fargate 的匹配度较高。

方案B的定位不是“全集团最终极架构”，而是：

> 大型企业部门级系统从企业验证走向正式生产时，在可靠性、扩展性、运维复杂度和成本之间较平衡的架构。

当前没有必要为了体现大型集团背景而立即采用：
...
- Organization / Department / Store 模型。
- 区域级数据隔离。
- 企业 SSO / IdP。
- 多个 API 与 Worker Task。
- Redis / 队列。
- 配额、部门预算和组织级 Audit。
- RDS Multi-AZ 与更强监控。
...
是否采用 EKS，应根据客户是否已有 Kubernetes 平台和 SRE 能力决定，而不是仅因为企业规模大。
...
PostgreSQL作为正式业务数据库
Repository隔离持久化实现
StorageService隔离Local/S3/MinIO
LLMProvider隔离外部LLM/Bedrock
容器化运行
配置与Secret外置
```
...
> ERIP 是面向大型流通集团的企业经营分析平台。本阶段以集团内部的区域经营分析部门或事业部为首期导入范围，按照大型企业部门级核心系统进行设计和实现。当前采用 React、FastAPI、PostgreSQL/pgvector、RAG、Approval、Persistent Audit 与 LLM Usage Ledger；生产部署优先采用 ECS Fargate、RDS PostgreSQL 和 Amazon S3，并通过 Repository、StorageService 与 LLMProvider 保留向事业公司级和集团级扩展的能力。

禁止夸大为：

```text
已经支持大型集团全部员工和全部门店同时使用
```
...
|---|---|
| 业务背景 | AEON级大型流通集团想定 |
| 当前首期范围 | 区域、事业部或经营分析部门 |
| 当前应用等级 | 大型企业部门级核心系统 |
| 也可对应 | 中型企业核心系统 |
| 是否小企业架构 | 否 |
| 是否集团全社级完成 | 否 |
| 推荐AWS基线 | 方案B：ECS Fargate + RDS + S3 |
| 未来演进 | 事业公司级 → 集团级 |
