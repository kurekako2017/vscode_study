...
输入：运行 `python3 main.py "上线后服务起不来怎么办"`；处理前数据是本地文档、Chunk 和原始问题。

```text
原始文档 -> load_documents() -> chunk_documents()
用户问题 -> rewrite_queries()：生成“部署失败 / 启动异常 / 回滚”等表达
│
▼
retrieve()：多查询召回 -> 合并去重
│
▼
rerank()：重新计算候选顺序 -> Top-K
│
▼
synthesize_answer()：根据 Top-K 生成答案和引用
```

节点对应：改写提高召回，检索产生候选，重排提高前几名质量，合成器只使用最终证据。最小输出是恢复建议和来源。
...
```bash
/usr/bin/python3 /home/victorkure/workspace/vscode_study/ai-lab/ai-learn/agent-advanced/projects/advanced_rag_pipeline_demo/main.py "LangGraph 适合什么场景，RAG 怎么做引用和重排？"
```

默认只打印检索结果和最终回答。如果你想看更多调试信息，可以加上 `--show-stats` 或 `--show-excerpt`：

```bash
/usr/bin/python3 /home/victorkure/workspace/vscode_study/ai-lab/ai-learn/agent-advanced/projects/advanced_rag_pipeline_demo/main.py "LangGraph 适合什么场景，RAG 怎么做引用和重排？" --show-stats --show-excerpt
```
...
- **使用者**：企业 RAG 开发者和检索质量优化人员。
- **要解决的问题**：对多文档进行切分、查询改写、多路召回、重排和带引用回答生成。
- **输入与输出**：输入用户问题和本地资料；输出排序后的证据、统计信息和引用型答案。
- **生产环境差距**：需要向量数据库、embedding/reranker 模型、增量索引、阈值标定和答案评估。
