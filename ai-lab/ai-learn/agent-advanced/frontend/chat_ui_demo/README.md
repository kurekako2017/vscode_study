...
输入：在聊天框提交一条消息；处理前数据是 React 表单状态、消息历史和 API 地址。

```text
用户输入 -> onSubmit()：阻止空消息并追加 user message
│
▼
fetch()：POST 到 Agent API
├── 成功 -> 解析响应 -> 追加 assistant message
└── 失败 -> 追加 error message
    │
    ▼
React 根据 messages 重新渲染对话和状态
```

节点对应：表单管理输入，Fetch 连接后端，消息 State 驱动 UI，错误分支提供反馈。最小输出是页面新增一条回答或错误提示。
