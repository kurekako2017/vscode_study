...
输入：`python3 eval_rag.py`；处理前数据是 `samples.json` 中的问题、参考关键词、期望来源和预测结果。

```text
samples.json -> load_samples()
│
▼
逐条读取 reference 与 prediction
├── 计算检索覆盖率
├── 计算答案关键词精度
└── 检查引用完整性
    │
...
节点对应：样本固定比较基准，各指标分别测检索、回答和引用，汇总用于版本对比。最小输出是逐样本分数和总体指标。
