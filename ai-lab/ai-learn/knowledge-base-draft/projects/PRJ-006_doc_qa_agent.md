...
- 项目 ID: PRJ-006
- main.py: `ai-learn/agent-lab/projects/doc_qa_agent/main.py`
- 定位: 小売業向け AI 経営分析システム
- 领域: RAG CLI
- 验证重点: 文档读取、Chunk、TopK、Prompt、Sources
- 关联知识点: KN-RAG-001, KN-RAG-002, KN-RAG-003, KN-RAG-004, KN-RAG-008

## 1. 项目背景

本项目小売業向け AI 経営分析システム的开发模块。它用于验证 `文档读取、Chunk、TopK、Prompt、Sources` 如何支撑小売業向け AI 経営分析システム的设计讨论、基本设计、详细设计、代码 Review 和面试说明。
...
```mermaid
graph TD
    A[业务输入] --> B[doc_qa_agent]
    B --> C[核心处理: 文档读取、Chunk、TopK、Prompt、Sources]
    C --> D[结构化结果/回答/事件]
    D --> E[开发记录]
    E --> F[運用版扩展评审]
```
...
sequenceDiagram
    ...
    participant App as doc_qa_agent
    participant Core as Core Logic
    participant Out as Output
    User->>App: 输入任务
    App->>Core: 执行 文档读取、Chunk、TopK、Prompt、Sources
    Core-->>App: 返回中间结果
    App-->>Out: 输出结果和验证信息
```
...
- 知识点: KN-RAG-001, KN-RAG-002, KN-RAG-003, KN-RAG-004, KN-RAG-008
- 项目索引: `02_INDEX_项目.md`
