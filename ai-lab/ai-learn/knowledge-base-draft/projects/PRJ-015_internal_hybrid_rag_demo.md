# PRJ-015 internal_hybrid_rag_demo

## 元数据

- 项目 ID: PRJ-015
- main.py: `ai-learn/agent-advanced/projects/internal_hybrid_rag_demo/main.py`
- 定位: 小売業向け AI 経営分析システム
- 领域: Hybrid RAG + ACL
- 验证重点: server docs、wiki docs、role ACL、rerank
- 关联知识点: KN-RAG-009, KN-RAG-007, KN-RAG-008

## 1. 项目背景

本项目小売業向け AI 経営分析システム的开发模块。它用于验证 `server docs、wiki docs、role ACL、rerank` 如何支撑小売業向け AI 経営分析システム的设计讨论、基本设计、详细设计、代码 Review 和面试说明。
...
```mermaid
graph TD
    A[业务输入] --> B[internal_hybrid_rag_demo]
    B --> C[核心处理: server docs、wiki docs、role ACL、rerank]
    C --> D[结构化结果/回答/事件]
    D --> E[开发记录]
    E --> F[運用版扩展评审]
```
...
sequenceDiagram
    ...
    participant App as internal_hybrid_rag_demo
    participant Core as Core Logic
    participant Out as Output
    User->>App: 输入任务
    App->>Core: 执行 server docs、wiki docs、role ACL、rerank
    Core-->>App: 返回中间结果
    App-->>Out: 输出结果和验证信息
```
...
- 知识点: KN-RAG-009, KN-RAG-007, KN-RAG-008
- 项目索引: `02_INDEX_项目.md`
