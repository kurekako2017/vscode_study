# PRJ-004 workflow_agent

## 元数据

- 项目 ID: PRJ-004
- main.py: `ai-learn/agent-lab/projects/workflow_agent/main.py`
- 定位: 小売業向け AI 経営分析システム
- 领域: Workflow Agent
- 验证重点: 分析、计划、总结三阶段工作流
- 关联知识点: KN-AGENT-003, DES-LANGGRAPH-002

## 1. 项目背景

本项目小売業向け AI 経営分析システム的开发模块。它用于验证 `分析、计划、总结三阶段工作流` 如何支撑小売業向け AI 経営分析システム的设计讨论、基本设计、详细设计、代码 Review 和面试说明。
...
```mermaid
graph TD
    A[业务输入] --> B[workflow_agent]
    B --> C[核心处理: 分析、计划、总结三阶段工作流]
    C --> D[结构化结果/回答/事件]
    D --> E[开发记录]
    E --> F[運用版扩展评审]
```
...
sequenceDiagram
    ...
    participant App as workflow_agent
    participant Core as Core Logic
    participant Out as Output
    User->>App: 输入任务
    App->>Core: 执行 分析、计划、总结三阶段工作流
    Core-->>App: 返回中间结果
    App-->>Out: 输出结果和验证信息
```
...
- 知识点: KN-AGENT-003, DES-LANGGRAPH-002
- 项目索引: `02_INDEX_项目.md`
