...
业务场景说明
- 谁会用：正在学习如何把 Python AI 程序变成后端服务的开发人员。
- 现实中的问题：本地 RAG 脚本虽然能回答问题，但 React 页面、Java 系统和其他电脑无法直接调用脚本里的函数。
- 这个例子怎么解决：通过 `/health` 查看服务状态，通过 `/reload` 重新加载文档，通过 `/ask` 发送问题并取得答案和来源。
- 现实例子：员工在 React 页面询问报销流程，页面把问题发送给 `/ask`，FastAPI 返回 JSON，页面再把答案和引用文件显示出来。
- 初学者重点：先理解“客户端发送请求 -> FastAPI 接收请求 -> 检索文档 -> 生成答案 -> 返回 JSON”。
...
RAG API 用于让 React、Java 或其他业务系统通过 HTTP 使用文档问答能力。输入是 JSON 请求，输出是结构化答案与来源；生产环境还需鉴权、限流、异步索引和可观测性。

## 整体流程图

```mermaid
flowchart TD
    A[业务客户端] --> B[FastAPI 接口]
    B --> C[参数校验]
    C --> D[文档检索]
    D --> E[模型或 Mock]
    E --> F[结构化响应]
