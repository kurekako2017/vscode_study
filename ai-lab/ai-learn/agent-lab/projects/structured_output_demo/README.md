...
- 现实中的问题：客服说“客户无法登录，影响很严重，请尽快处理”。如果模型只返回一段自由文字，程序很难稳定取出问题类型、紧急程度和处理步骤。
- 这个例子怎么解决：用 Pydantic 规定模型必须返回哪些字段以及每个字段的数据类型。程序收到结果后会先校验，字段不完整或类型不正确时就能及时发现。
- 现实例子：把“下周完成登录故障调查并提交报告”转换成包含 `title`、`summary`、`steps` 和 `risks` 的固定数据，之后可以直接写入任务管理系统。
- 初学者重点：普通输出主要给人阅读，结构化输出主要给程序读取和继续处理。
