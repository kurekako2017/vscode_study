...
如果你在 RAG 这一章先想建立整体图景，可以先看 [../04-RAG.md](../04-RAG.md) 里的流程图。

如果你想先抓住 Agent 的三种典型形态，可以先按下面这条线理解：

| 形态 | 代表文件 | 适合记住什么 |
| --- | --- | --- |
| 总架构 | [../Agent系统框架与术语.md](../Agent系统框架与术语.md) | Agent 的角色分工和整体组成 |
| 固定工作流 | [../05-Agent工作流.md](../05-Agent工作流.md) | 先分析，再计划，再总结 |
| 工具调用闭环 | [../03-Tool%20Calling.md](../03-Tool%20Calling.md) | 模型选择工具，程序执行工具 |
...
- 你先学什么：模型输出怎么变成稳定的结构化数据。
- 为什么先学：后面的工作流和计划阶段都需要 schema 约束。
- 看哪份文档：`structured_output_demo/README_LEARN.md`。
...
- 你先学什么：Analyze -> Plan -> Finalize 怎么串起来。
- 为什么先学：固定工作流是理解 Agent 编排的最稳入口。
- 看哪份文档：`workflow_agent/README_LEARN.md`，以及 [../05-Agent工作流.md](../05-Agent工作流.md) 的图。

### 快速运行

```bash
cd ai-learn/agent-lab/projects/workflow_agent
...
- 你先学什么：模型怎么选择工具、程序怎么执行工具。
- 为什么先学：这是 Agent 从“只会回答”变成“能动手做”的关键一步。
- 看哪份文档：`tool_agent_demo/README_LEARN.md`，以及 [03-Tool Calling.md](../03-Tool%20Calling.md) 的流程图。

### 快速运行

```bash
cd ai-learn/agent-lab/projects/tool_agent_demo
...
- 你先学什么：怎么从本地文档里找证据再回答。
- 为什么先学：RAG 是很多 Agent 的资料底座，也是工具调用的常见来源。
- 看哪份文档：`doc_qa_agent/README_LEARN.md`，以及 [../04-RAG.md](../04-RAG.md) 的图。

### 快速运行

```bash
cd ai-learn/agent-lab/projects/doc_qa_agent
...
- 你先学什么：怎么把 RAG 变成可调用的后端服务。
- 为什么先学：这是从 demo 走向真实系统集成的第一步。
- 看哪份文档：`rag_api_demo/README_LEARN.md`，以及 `rag_api_demo/run-dev.sh` / `rag_api_demo/mock_test.py`。
