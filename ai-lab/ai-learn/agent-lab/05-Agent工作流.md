...
如果要先建立对 `RAG` 的整体感，可以先看 [04-RAG.md](./04-RAG.md) 里的知识更新 / 知识检索流程图。
...
![Agent 工作流原图](./assets/agent_workflow_original.png)

## 5.1 用“打车去北京旅游”理解这张图

如果把这张工作流图放到一个生活场景里，可以这样理解：

```text
目标：去北京旅游，并且尽量少走弯路、少花冤枉钱
```

对应关系如下：

| 图中角色 / 流程 | 例子里的作用 | 具体会做什么 |
| --- | --- | --- |
| 输入层 | 你提出旅游目标 | 比如“下周去北京玩 3 天，预算 3000 元” |
| `Analyzer` | 先理解需求 | 判断是自由行、跟团游，是否要看预算、天气、景点距离 |
| `State / analysis` | 保存分析结果 | 记住“3 天、预算 3000、想看故宫和博物馆” |
| `Planner` | 制定计划 | 安排第 1 天住哪，第 2 天去哪，怎么坐车最省时间 |
| `Schema / WorkflowPlan` | 约束计划格式 | 规定计划里必须有日期、地点、交通方式、预算 |
| `State / plan` | 保存结构化计划 | 把“景点顺序、交通方式、预算分配”存起来 |
| `Finalizer` | 汇总建议 | 输出一份可执行的旅游安排清单 |
| `Executor` | 真正执行动作 | 如果在完整系统里，可能会订车票、查地铁、订酒店 |
| `Termination` | 判断结束 | 当计划生成完、检查通过后，就结束流程 |

如果把它再拆细一点：

1. `Analyzer` 先问清楚你要几天、预算多少、想玩什么。
2. `Planner` 根据这些信息安排路线，比如先去天安门，再去故宫，再去颐和园。
3. `State` 负责记住前面已经确认过的信息，避免后面重复问。
4. `Executor` 负责把“查车次、订酒店、导航”这些动作真正做掉。
5. `Finalizer` 把结果整理成你能直接照着执行的旅游计划。

这个例子里最重要的是：

- `Planning` 解决“先做什么、后做什么”
- `Memory / State` 解决“前面已经确认过什么”
- `Action / Executor` 解决“把计划变成真实动作”
- `Reflection` 解决“计划是否合理，需不需要改”

如果你对比 `tool_agent_demo` 来看，可以把“查车次、订酒店、查地图”理解成工具调用；如果对比 `workflow_agent` 来看，这里更像固定流程的旅行规划器。

## 5.2 把整张图按顺序跑一遍

如果把图里的流程连起来，它大致会这样发生：

```text
1. 你提出目标：去北京旅游
2. 系统先分析：几天、预算、偏好、限制
3. 系统把分析结果存进记忆
4. 系统制定计划：先住哪、先去哪、怎么走
5. 系统调用工具：查天气、查车次、查地图、查酒店
6. 工具返回结果后，系统把结果再存进记忆
7. 系统检查计划是否合理，必要时调整
8. 最后输出可执行的旅游方案
```

这条线对应到 Agent 的通用理解，就是：

- `Planning` 负责“想清楚”
- `Memory` 负责“记下来”
- `Tools` 负责“查和算”
- `Action` 负责“真的去做”
- `Reflection` 负责“发现问题再改”
...
对应到 `ai-learn/agent-lab/projects/workflow_agent/main.py`：
