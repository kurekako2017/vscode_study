...
| 术语 | 日本語 / 现场说法 | 概念性理解 | 在项目里的学习位置 |
| --- | --- | --- | --- |
| LLM | 大規模言語モデル / LLM | Large Language Model，大语言模型。它擅长根据上下文生成语言或结构化内容 | `02-模型调用基础.md` |
| Token | トークン | 模型处理文本的基本单位，近似“字/词片段” | 模型成本、上下文长度相关 |
| Context Window | コンテキストウィンドウ | 模型一次能看到的最大上下文范围 | RAG 和长文处理必须考虑 |
| Prompt | プロンプト | 给模型的输入说明，不只是问题，也包括规则、示例、资料 | `02-模型调用基础.md` |
| Temperature | 温度パラメータ | 控制输出随机性。低温更稳定，高温更发散 | 模型参数控制 |
| Structured Output | 構造化出力 | 让模型输出固定 JSON 或 schema | `03-结构化输出.md` |
| Pydantic | データ検証ライブラリ | Python 数据校验库，用来定义字段和校验结果 | `03-结构化输出.md` |
| Embedding | エンベディング / ベクトル化 | 把文本变成向量，用于相似度检索 | `04-RAG.md` |
| RAG | 検索拡張生成 / RAG | Retrieval-Augmented Generation，先检索资料，再让模型回答 | `04-RAG.md` |
| Chunk | チャンク | 文档切分后的片段 | RAG 文档处理 |
| Retriever | リトリーバー / 検索器 | 检索器，负责找相关 chunk | RAG 主流程 |
| Citation | 引用元 / 出典 | 引用来源，说明答案依据哪段资料 | 企业问答可信度关键 |
| Eval | 評価 | 评估，用测试问题集衡量质量 | `06-评估与运维.md` |
| Latency | レイテンシ | 延迟，请求到响应的耗时 | 运维和用户体验 |
| Cost | コスト | 成本，通常和 token、模型、请求量有关 | 运维与上线评估 |

更完整的中文、English、日本語对照见：
