...
- 适用场景：开发阶段快速验证对话入口、参数解析、Mock / Real 切换，以及最小聊天闭环。
- 如果不用这种方式：每次都要依赖更复杂的前端或服务外壳，调试控制流和回归行为会更慢。
- 解决的问题：把“命令行输入 -> 模型调用 -> 输出结果”先跑通，方便做本地调试、教学演示和自动化测试。
- 举例说明：例如先用 `--mock` 跑通“用一句话解释什么是 agent”，确认命令行输入、输出格式和模式切换都正常后，再切到真实 API。
