...
- 适用场景：用户需要在浏览器里直接和 RAG 服务交互，查看回答、来源和历史记录。
- 如果不用这种方式：只有后端接口的话，业务人员很难直观看到效果，也不利于演示和验收。
- 解决的问题：提供一个最小但完整的前端壳子，让后端能力可以被真实用户体验和验证。
- 举例说明：例如在前端输入“报销流程怎么走”，确认浏览器界面能收到后端返回的检索结果和来源。
