...
- 适用场景：需要把 RAG 能力暴露成 HTTP 服务，给前端页面、命令行、浏览器或其他系统统一调用。
- 如果不用这种方式：RAG 只能停留在脚本里，无法和真实业务系统集成，也不方便做部署和权限控制。
- 解决的问题：把文档检索、模型问答和接口层拆开，形成可复用的后端服务，支持前后端分离和系统集成。
- 举例说明：例如先接入一份内部制度文档，再问“报销流程怎么走”，看检索结果和答案是否一致。
