...
- 适用场景：当下游系统要把模型结果直接接到审批、工单、分类、任务编排或 API 返回里时。
- 如果不用这种方式：输出格式会不稳定，后端很难可靠解析，稍微一变就可能导致流程失败。
- 解决的问题：用 schema 约束模型输出，让结果可校验、可消费、可自动化处理。
- 举例说明：例如先让模型把一段自然语言整理成固定字段的 JSON，确认结构化输出能被程序直接使用。
