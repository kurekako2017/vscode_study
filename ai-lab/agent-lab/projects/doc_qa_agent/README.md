...
- 适用场景：公司内部文档问答、制度检索、项目资料查询这类“先找资料再回答”的任务。
- 如果不用这种方式：模型只能凭记忆回答，容易答错、漏答或编造，尤其在文档经常更新时更明显。
- 解决的问题：先检索本地资料，再把命中的内容交给模型生成回答，并保留来源，减少幻觉并提高可追溯性。
- 举例说明：例如先放一份请假制度或项目说明书，再问“请假流程怎么走”，验证先检索再回答的闭环。
