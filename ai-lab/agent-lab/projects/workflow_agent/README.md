...
- 适用场景：任务天然分阶段、需要先分析再计划再总结的内部流程，例如 PoC 方案编写、需求拆解或发布准备。
- 如果不用这种方式：把所有逻辑塞进一次调用里，容易漏掉约束，结果也更难复现和审阅。
- 解决的问题：用固定工作流把复杂任务拆开，保证过程可控、状态可追踪、输出更稳定。
- 举例说明：例如把需求拆成“分析 -> 计划 -> 执行”三步，看看每一步的中间状态是否都能保留下来。
