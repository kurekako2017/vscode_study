...
- 适用场景：文档很多、问题较复杂、单纯关键词检索不够准的知识库问答或社内搜索场景。
- 如果不用这种方式：简单 RAG 很容易召回偏题片段，答案会片面、遗漏，引用质量也不稳定。
- 解决的问题：把查询改写、检索、重排和引用拆成流水线，提高召回准确率和回答可信度。
- 举例说明：例如先用一份内部政策文档跑完整流水线，看看加载、切分、检索、重排和回答是否都能正常工作。
