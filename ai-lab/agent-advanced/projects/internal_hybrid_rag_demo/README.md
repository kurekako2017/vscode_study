...
- 适用场景：企业内部资料分散在文件服务器和 Wiki，且不同角色能看到的内容不一样。
- 如果不用这种方式：员工可能搜不到该看的资料，或者把无权限内容直接带进答案，造成错误甚至泄露。
- 解决的问题：统一接入多来源资料，再叠加权限过滤和引用输出，适合真正的社内搜索和支持场景。
- 举例说明：例如同时放入公司 PDF 和 Wiki 页面，再问“请假与报销规则是什么”，确认混合检索能汇总两类来源。
