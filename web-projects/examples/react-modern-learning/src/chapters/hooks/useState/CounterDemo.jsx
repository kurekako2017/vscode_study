...
export default function CounterDemo() {
  useLearningLifecycle('CounterDemo')

  // count 保存当前计数值，是最基础的组件状态示例。
  const [count, setCount] = useLearningState('CounterDemo', 'count', 0)
  // step 保存每次点击按钮时增加多少，顺便演示表单控件和 state 的联动。
  const [step, setStep] = useLearningState('CounterDemo', 'step', 1)

  // doubled 不是独立状态，而是由 count 推导出来的结果。
  // 这里用 useMemo 只是为了说明“派生值可以按依赖缓存”。
  // 初学者可以先把它理解成“计算结果缓存”，而不是新的数据源。
  const doubled = useMemo(() => count * 2, [count])
  useLearningHookSnapshot('CounterDemo', 'useMemo()', doubled, `doubled=${doubled}`)
    ...
    <article className="card stack">
      ...
      <label className="stack">
        ...
        <select
          value={step}
          onChange={(event) => {
            traceEvent('Step change')
            traceCall('handleStepChange()')
