...
| Ditto| 它是一个确定性网站克隆器（不完全依赖 LLM，而是结构化解析），输出干净的 Vite 或 Next.js 代码。|
...
- 用 Markdown 编写 Design System，让 Claude Code、Codex、Cursor、Gemini CLI 等直接理解设计规范。
- 适合：
  - React
  - Vue
  - Next.js
...
第一阶段：Git、GitHub、Markdown、MarkItDown

第二阶段：React、TypeScript、awesome-design-md

第三阶段：Claude Code、Codex、Cursor、OpenHands

第四阶段：browser-use、agency-agents、RAGFlow、ZVEC

第五阶段：MCP、LangGraph、企业级 Agent

## 21. GitHub 工具学习体系维护规范

以后发现新的 GitHub 工具时，先在本学习地图中登记工具名称、仓库链接、用途、学习价值和许可证，再为它建立独立目录。全部教程、案例、问题记录和学习笔记使用中文。
...
不得用一份超长文档代替全部内容。安装、日常使用、实战、排错和个人总结分别维护，方便以后更新。

### 21.2 平台适配要求

每个工具都要说明下列环境的支持情况、差异和注意事项：
...
工具不支持某个平台时，要明确写“暂不支持”并说明可行替代方案，不得省略。

### 21.3 模型优先级

需要大语言模型的工具，默认按以下顺序配置：

1. OpenRouter
2. ChatNVIDIA
3. Ollama（`Qwen2.5-Coder:1.5B`）

教程只写环境变量名称和无效占位值，不得记录真实 API Key。使用 Ollama 时要说明本机资源要求和模型能力边界。

### 21.4 每份教程的必备内容

每份教程都必须解释：

- 为什么学：这个工具解决什么问题，为什么值得投入时间。
- 学什么：本教程的知识点、目标和最终成果。
- 如何安装：依赖、版本、平台差异和完整步骤。
- 如何运行：最小可运行命令、配置和预期输出。
- 如何验证：健康检查、测试命令或人工验收方法。
- 如何结合 AI-Learn 项目：明确对应 `ai-lab/ai-learn` 的练习、模块或可复用场景。

### 21.5 五类文档的职责

| 文档 | 主要内容 |
| --- | --- |
| `安装教程.md` | 前置条件、各平台安装、模型配置、首次启动和安装验证 |
| `使用教程.md` | 核心概念、常用命令、配置方式、基础操作和运行验证 |
| `实战案例.md` | 一个能完成的真实任务、输入输出、步骤、测试及与 AI-Learn 的集成 |
| `常见问题.md` | 错误现象、可能原因、排查步骤、解决办法和验证方法 |
| `学习笔记.md` | 学习日期、版本、关键认识、踩坑记录、待办事项和后续复习结论 |

### 21.6 新工具登记与验收流程

1. 确认官方仓库、许可证、维护活跃度和学习价值。
2. 在本地图对应分类登记，避免重复收录。
3. 创建标准的五文件目录，不使用英文教程名。
4. 优先完成最小安装和运行验证，再编写实战案例。
5. 补齐五个平台的支持说明和模型优先级。
6. 说明如何结合 `ai-lab/ai-learn`。
7. 检查文档中没有真实密钥、Token、账号或个人资料。
8. 更新学习笔记和后续计划。

只有五类文档齐全、最小示例可验证、平台差异明确、敏感信息检查通过后，才标记为“学习体系已建立”。
